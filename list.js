...
function dirAddButtonClick() {
    ...
    writeDB(data, 'dirStore', function (d_id) {
        ...
        const extensionURL = chrome.runtime.getURL(
            `list.html?dir_id=${dir_id}&dirname=${encodeURIComponent(dirName)}`
        );
    ...
    readValueDB('userHistoryStore', 'nowExecutedDir', 1)
        .then((result) => {
            ...
            if (parseInt(result) == d_id) {
                ...
                const dirCannotDeleteModal = document.getElementById(
                    'dirCannotDeleteModal'
                );
                dirCannotDeleteModal.style.display = 'flex';
                const dirCannotDeletecloseBtn =
                    dirCannotDeleteModal.querySelector('.closeBtn');
            ...
            //현재 실행 중인 dir과 삭제하려는 dir이 동일하지 않다면 "진짜 삭제하실?"
                ...
                const dirDeleteModal =
                    document.getElementById('dirDeleteModal');
                dirDeleteModal.style.display = 'flex';
                const dirDeleteModalcloseBtn =
                    dirDeleteModal.querySelector('.closeBtn');
                const dirDeleteModalDeleteBtn =
                    dirDeleteModal.querySelector('.deleteBtn');
    ...
    if (dirId != null) {
        ...
        try {
            ...
            if (initDirName == undefined) {
                ...
                dirData = await readDBbyStoreNameAndId(
                    'dirStore',
                    parseInt(dirId)
                );
            }
...
function startBtnEvent() {
    ...
    content.innerHTML =
        '<b>[' + dirName + ']</b> <br>폴더에 검색 기록이 저장됩니다.';
    modal.classList.replace('hidden', 'clicked');
    setTimeout(() => {
        modal.classList.replace('clicked', 'hidden');
    }, 5000);
...
startButton.addEventListener('click', () => {
    ...
    chrome.runtime.sendMessage({
        onDirId: nowDirId,
        txt: 'Start the extension from list.js',
    });
...
function editDirNameDB(id, name) {
    ...
    request.onsuccess = function (event) {
        ...
        requestId.onerror = function (event) {
...
function deleteDirDB(id) {
    ...
    request.onsuccess = function (event) {
        ...
        const keywordObjectStore =
            keywordTransaction.objectStore('keywordStore');
        const keywordSearch = keywordObjectStore
            .index('dir_id')
            .getAll(parseInt(id));
        console.log('여기');
        ...
        keywordSearch.onsuccess = (e) => {
            ...
            for (let i = 0; i < keywords.length; i++) {
                deleteDB('keywordStore', keywords[i].id, false);
            }
        };
        ...
        urlSearch.onsuccess = (e) => {
            ...
            for (let i = 0; i < urls.length; i++) {
                deleteDB('urlStore', urls[i].id, false);
            }
        };
...
function displayURL(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        const memoIcon = clone
            .querySelector('.path-box')
            .querySelector('.memoIcon');
...
function displayKeyword(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        clone
            .querySelector('.keyword-box')
            .querySelector('.keyword').innerHTML = k;
        clone.querySelector('.keyword-box').id = 'green-' + k;
        clone.querySelector('.path-area').id = 'white-' + k;
        const toggleButton = clone
            .querySelector('.keyword-box')
            .querySelector('.toggle_keyword');
        ...
        request.onsuccess = function (event) {
            ...
            greenIndex.openCursor(greenKeyRange).onsuccess = function (event) {
                ...
                if (cursor) {
                    ...
                    dirFilterKeyword.push([
                        cursor.value.id,
                        cursor.value.keyword,
                        cursor.value.isToggled,
                    ]);
                    cursor.continue();
                } else {
        ...
        request.onsuccess = function (event) {
            ...
            requestKey.onsuccess = function (event) {
                ...
                for (var i = 0; i < keyData.length; i++) {
                    ...
                    let requestUrlSearch = uStore
                        .index('dir_id_keyword')
                        .get([dirId, keyword]);
...
async function displayData(data) {
    ...
    data.forEach((item) => {
        const copy = document
            .getElementById('dir_template')
            .content.cloneNode(true);
        const dirList = copy.querySelector('.dir-list');
        const editButton = copy.querySelector('.button-edit');
        const deleteButton = copy.querySelector('.button-del');
        const dirLink = copy.querySelector('.dir-link');
        const dirTooltip = copy
            .querySelector('.dir-list')
            .querySelector('.tooltipDirTitle');
        ...
        const extensionURL = chrome.runtime.getURL(
            `list.html?dir_id=${item.d_id}&dirname=${encodeURIComponent(
                item.dir_name
            )}`
        );
        dirLink.href = extensionURL;
...
function deleteDB(obs, key, reload) {
    // 1. db 열기
    var request = indexedDB.open('HeyGoogler', 1);
